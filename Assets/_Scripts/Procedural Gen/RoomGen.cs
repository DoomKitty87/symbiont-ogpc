...
public class RoomGen : MonoBehaviour
{
  ...
  private void GenerateRoom() {
    ...
    Collider[] cols = Physics.OverlapSphere(instantiatedRoom.transform.position, instantiatedRoom.GetComponent<Renderer>.bounds.x / 2, roomLayer);

    while (cols.Length > 1) {
      instantiatedRoom.transform.RotateAround(lastRoom.transform.GetChild(1).GetChild(chosenDoor).position, Vector3.up, 180);
      cols = Physics.OverlapSphere(instantiatedRoom.transform.position, instantiatedRoom.GetComponent<Renderer>.bounds.x / 2, roomLayer);
    }

    loop:
      foreach (GameObject room in generatedRooms) {
        cols = Physics.OverlapSphere(room.transform.position, room.GetComponent<Renderer>.bounds.x / 2, roomLayer);
        if (cols.Length > 1) {
