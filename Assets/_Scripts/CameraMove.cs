...
public class CameraMove : MonoBehaviour
{
  ...
  private void CheckForTarget() {
	...
	// Put in check for if a target is 'teleportable'
	  ...
	  if (Physics.Raycast(transform.position, transform.TransformDirection(Vector3.forward), out RaycastHit hit, maxTeleportDistance)) {
  ...
  private void TeleportToTarget(GameObject target) {
    ...
    _currActiveTarget = target.GetComponent<TargetHolder>().targetHead;
  }
}
