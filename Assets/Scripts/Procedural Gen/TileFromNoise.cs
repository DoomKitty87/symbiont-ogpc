  ...
  private void UpdateMeshVertices(float[,] heightMap) {
    ...
    for (int zIndex = 0; zIndex < tileDepth; zIndex++) {
      for (int xIndex = 0; xIndex < tileWidth; xIndex++) {
        ...
        meshVertices[vertexIndex] = new Vector3(vertex.x, heightCurve.Evaluate(height) * heightMultiplier * Mathf.SmoothStep(heightIncStart, heightIncEnd, ((float)xIndex) / (tileWidth - 1)), vertex.z);
        vertexIndex++;
      }
    }
